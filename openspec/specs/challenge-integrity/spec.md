...
## Purpose
Keep the CTF-1 challenge solvable and honest: the documented walkthrough path must actually work end to end, credentials must be classified truthfully (a load-bearing key is never labeled a red herring), the build must not emit recurring runtime errors, and version pins must stay reproducible.
